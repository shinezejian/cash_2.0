								...
								<tbody>
									<c:forEach items="${user.roles}" var="role" varStatus="vs">
										<tr data-tt-id="${vs.index+1}" data-tt-parent-id="0">
											<td>角色：<a href="${pageContext.request.contextPath}/role/findById.do?id=${role.id}">${role.roleName}</a></td>
											<td>描述：${role.roleDesc }</td>
										</tr>
										<c:forEach items="${role.permissions}" var="permission">
											<tr data-tt-id="1-1" data-tt-parent-id="${vs.index+1}">
												<td>权限：<a href="${pageContext.request.contextPath}/permission/findById.do?id=${permission.id}">${permission.permissionName}</a></td>
												<td>描述：${permission.url}</td>
											</tr>
